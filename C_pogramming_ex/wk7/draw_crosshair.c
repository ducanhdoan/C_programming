// Implement a cross hair controlled by the joystick.
...
void process( void ) {
	...
	// LCD_CMD(lcd_set_x_addr, XC);
	// LCD_CMD(lcd_set_y_addr, YC);
	int bank= YC/8;//double to int will floor automatically.
	...
	//	variable pixel_pos.
	uint8_t new_bank=YC/8;
	uint8_t pixel_pos=YC%8;
	...
	uint8_t bit_pattern= 0b00000001 << pixel_pos;
